...
if ($action === 'bukti') {
    ...
    $bd = []; $ba = [];
    $r = mysqli_query($koneksi, "SELECT id_bd, bukti_dasar FROM tb_bukti_dasar WHERE id_skema = '$id_skema' ORDER BY id_bd ASC");
    while ($row = mysqli_fetch_assoc($r)) $bd[] = $row;
    $r = mysqli_query($koneksi, "SELECT id_ba, bukti_adm FROM tb_bukti_adm WHERE id_skema = '$id_skema' ORDER BY id_ba ASC");
    while ($row = mysqli_fetch_assoc($r)) $ba[] = $row;
    echo json_encode(['status' => 'ok', 'bukti_dasar' => $bd, 'bukti_adm' => $ba]);
    exit;
}
...
<div class="form-box">
    ...
    <div style="display:flex; flex-direction:column; gap:13px;">
        <div style="border:1px solid #ddd; border-radius:5px; padding:12px 14px; background:#fafbff;">
            <div class="label" style="margin-bottom:8px;">Skema Sertifikasi <span style="font-size:12px;color:#555;">(KKNI/Okupasi/Klaster)</span></div>
        ...
        <div>
            <div class="label" style="margin-bottom:6px;">Tujuan Asesmen</div>
            <div style="padding-left:4px; font-size:14px; pointer-events:none; opacity:0.8;">
                <?php foreach(['Sertifikasi','Pengakuan Kompetensi Terkini (PKT)','Rekognisi Pembelajaran Lampau (RPL)','Lainnya'] as $opt): ?>
                <label style="display:block; margin-bottom:4px;">
                    <input type="radio" <?= $data_apl1['tujuan_asesmen'] === $opt ? 'checked' : '' ?> disabled>
                    &nbsp;<?= h($opt) ?>
                    <?php if($opt==='Lainnya' && $data_apl1['tujuan_lainnya']): ?>
                    : <b><?= h($data_apl1['tujuan_lainnya']) ?></b>
                    <?php endif; ?>
                </label>
                <?php endforeach; ?>
            </div>
        ...
        <div>
            ...
            <?php
            $res_unit_v = mysqli_query($koneksi,
                "SELECT u.kode_unit, u.judul_unit, s.standar_kompetensi_kerja
                 FROM tb_unit_kompetensi u
                 JOIN tb_skema s ON s.id_skema = u.id_skema
                 WHERE u.id_skema='$id_skema_view' ORDER BY u.id_unit ASC");
            $unit_rows = [];
            while($ur = mysqli_fetch_assoc($res_unit_v)) $unit_rows[] = $ur;
            ?>
            <div style="overflow-x:auto;">
            <table class="tbl-unit">
                <thead><tr><th>No.</th><th>Kode Unit</th><th>Judul Unit</th><th>Standar Kompetensi Kerja</th></tr></thead>
                <tbody>
                <?php foreach($unit_rows as $i => $ur): ?>
                <tr>
                    <td><?= $i+1 ?></td>
                    <td><?= h($ur['kode_unit']) ?></td>
                    <td><?= h($ur['judul_unit']) ?></td>
                    <?php if($i===0): ?>
                    <td rowspan="<?= count($unit_rows) ?>" style="text-align:center;vertical-align:middle;font-size:12px;">
                        <?= h($ur['standar_kompetensi_kerja']) ?>
                    </td>
                    <?php endif; ?>
                </tr>
                <?php endforeach; ?>
    ...
    <!-- BAGIAN 3 -->
    <div class="section-title" style="margin:26px 0 10px 0;">Bagian 3 : Bukti Kelengkapan Pemohon</div>
    <div style="margin-bottom:16px;">
        <div class="label" style="margin-bottom:6px; font-size:14px;">3.1 Bukti Persyaratan Dasar Pemohon</div>
        <div style="overflow-x:auto;">
        <table class="tbl-bukti">
            <thead>
                <tr><th rowspan="2">No.</th><th rowspan="2">Bukti Persyaratan Dasar</th><th colspan="2">Ada</th><th rowspan="2">Tidak Ada</th></tr>
                <tr><th>Memenuhi Syarat</th><th>Tidak Memenuhi Syarat</th></tr>
            </thead>
            <tbody>
            <?php
            $res_bd_v = mysqli_query($koneksi,
                "SELECT bd.id_bd, bd.bukti_dasar, ibd.kondisi
                 FROM tb_bukti_dasar bd
                 LEFT JOIN tb_isi_bukti_dasar ibd ON ibd.id_bd=bd.id_bd AND ibd.id_asesi='$id_asesi'
                 WHERE bd.id_skema='$id_skema_view' ORDER BY bd.id_bd ASC");
            $no=1;
            while($bd=mysqli_fetch_assoc($res_bd_v)):
                $kondisi = $bd['kondisi'] ?? '';
            ?>
            <tr>
                <td><?= $no++ ?>.</td>
                <td style="text-align:left;"><?= h($bd['bukti_dasar']) ?></td>
                <td style="text-align:center;pointer-events:none;">
                    <input type="radio" <?= $kondisi==='Memenuhi Syarat' ? 'checked' : '' ?> disabled>
                </td>
                <td style="text-align:center;pointer-events:none;">
                    <input type="radio" <?= $kondisi==='Tidak Memenuhi Syarat' ? 'checked' : '' ?> disabled>
                </td>
                <td style="text-align:center;pointer-events:none;">
                    <input type="radio" <?= $kondisi==='Tidak Ada' ? 'checked' : '' ?> disabled>
                </td>
            </tr>
            <?php endwhile; ?>
            </tbody>
    ...
    <div style="margin-bottom:16px;">
        ...
        <table class="tbl-bukti">
            <thead>
                <tr><th rowspan="2">No.</th><th rowspan="2">Bukti Administratif</th><th colspan="2">Ada</th><th rowspan="2">Tidak Ada</th></tr>
                <tr><th>Memenuhi Syarat</th><th>Tidak Memenuhi Syarat</th></tr>
            </thead>
            <tbody>
            <?php
            $res_ba_v = mysqli_query($koneksi,
                "SELECT ba.id_ba, ba.bukti_adm, iba.kondisi
                 FROM tb_bukti_adm ba
                 LEFT JOIN tb_isi_bukti_adm iba ON iba.id_ba=ba.id_ba AND iba.id_asesi='$id_asesi'
                 WHERE ba.id_skema='$id_skema_view' ORDER BY ba.id_ba ASC");
            $no=1;
            while($ba=mysqli_fetch_assoc($res_ba_v)):
                $kondisi = $ba['kondisi'] ?? '';
            ?>
            <tr>
                <td><?= $no++ ?>.</td>
                <td style="text-align:left;"><?= h($ba['bukti_adm']) ?></td>
                <td style="text-align:center;pointer-events:none;">
                    <input type="radio" <?= $kondisi==='Memenuhi Syarat' ? 'checked' : '' ?> disabled>
                </td>
                <td style="text-align:center;pointer-events:none;">
...
<form method="post" autocomplete="off" id="mainForm">
    ...
    <div style="margin-bottom:16px;">
        <div class="label" style="margin-bottom:6px; font-size:14px;">3.1 Bukti Persyaratan Dasar Pemohon</div>
        <div style="overflow-x:auto;">
            <table class="tbl-bukti"  id="tbl-bukti-dasar">
                <thead>
                    <tr>
                        <th rowspan="2">No.</th>
    ...
    <div style="margin-bottom:16px;">
        <div class="label" style="margin-bottom:6px; font-size:14px;">Bukti Administratif</div>
        <div style="overflow-x:auto;">
            <table class="tbl-bukti" id="tbl-bukti-adm">
                <thead>
                    <tr>
                        <th rowspan="2">No.</th>

...
$skema_asesor = mysqli_fetch_assoc(mysqli_query($koneksi,
    "SELECT ar.nama_asesor, ar.no_reg
     ...
     WHERE a.id_apl1='$id_apl1' LIMIT 1"));
$nama_asesor = $skema_asesor['nama_asesor'] ?? '-';
$no_reg_asesor = $skema_asesor['no_reg'] ?? '-';


$id_validator = isset($_GET['id_validator']) ? intval($_GET['id_validator']) : 0;
...
$val = mysqli_fetch_assoc(mysqli_query($koneksi,
    "SELECT v.username, v.noreg
     FROM tb_validator v
     WHERE v.id_validator='$id_validator' LIMIT 1"));
    ...
    <div class="page">
        ...
        <table class="validator-table">
            ...
            <tbody>
                ...
                <tr><td>VALIDATOR</td><td><?= h($username) ?></td><td><?= h($noreg) ?></td><td style="height:50px;"></td></tr>
            </tbody>
        </table>
    </div>
</div>

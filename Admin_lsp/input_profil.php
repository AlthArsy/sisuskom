...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'Admin_lsp') {
    echo "<script>alert('Akses ditolak! Silakan login sebagai Admin LSP.'); window.location.href='../LOGIN/login.php';</script>";
    exit();
}
include '../koneksi.php';
...
$id_admin = null;
$stmt_check = mysqli_prepare($koneksi, "SELECT id_admin FROM users WHERE id_user = ? LIMIT 1");
if ($stmt_check) {
    mysqli_stmt_bind_param($stmt_check, 'i', $id_user);
    mysqli_stmt_execute($stmt_check);
    mysqli_stmt_bind_result($stmt_check, $id_admin);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nik && $nama_admin) {
        $sql = "INSERT INTO tb_admin (nik, nama_admin) VALUES (?, ?)";
        $stmt = mysqli_prepare($koneksi, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, 'ss', $nik, $nama_admin);
            if (mysqli_stmt_execute($stmt)) {
                $id_admin = mysqli_insert_id($koneksi);
                mysqli_stmt_close($stmt);
                if ($id_admin > 0) {
                    $stmt_up = mysqli_prepare($koneksi, "UPDATE users SET id_admin = ? WHERE id_user = ?");
                    if ($stmt_up) {
                        mysqli_stmt_bind_param($stmt_up, 'ii', $id_admin, $id_user);
                        if (mysqli_stmt_execute($stmt_up)) {
                            $_SESSION['id_admin'] = $id_admin;
                            $_SESSION['id_referensi'] = $id_admin;
                    ...
                    echo "<script>alert('Profil tersimpan, tetapi gagal mengambil ID profil (insert_id). Pastikan kolom id_admin AUTO_INCREMENT.');</script>";
                }
            } else {

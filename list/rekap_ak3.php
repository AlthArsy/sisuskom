<?php

include "../koneksi.php";
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

$role = $_SESSION['role'] ?? '';
if (!in_array($role, ['Asesor', 'Admin_lsp', 'Admin_utm'])) {
    echo "<p style='color:red;padding:20px;'>Akses ditolak.</p>";
    exit;
}

// if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status_ak03'])) {
//     $id_ak03 = intval($_POST['id_ak03']);
//     // $status  = $_POST['status_ak03'] ?? '';
//     $catatan = $_POST['catatan_ak03'] ?? '';

//     // if (in_array($status, ['Sesuai', 'Tidak Sesuai'])) {
//     //         "UPDATE tb_ak03 SET status=?, catatan_admin=? WHERE id_ak03=?");
//     //   //     $stmt = mysqli_prepare($koneksi,
//       mysqli_stmt_bind_param($stmt, "ssi", $status, $catatan, $id_ak03);
//     // } else {
//     //     $stmt = mysqli_prepare($koneksi,
//             "UPDATE tb_ak03 SET status=NULL, catatan_admin=? WHERE id_ak03=?");
//         mysqli_stmt_bind_param($stmt, "si", $catatan, $id_ak03);
//     }
//     mysqli_stmt_execute($stmt);

//     $filter_param = isset($_GET['filter']) ? '&filter=' . urlencode($_GET['filter']) : '';
//     header("Location: {$base}?page=../list/rekap_ak3.php{$filter_param}");
//     exit; 
// }

if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_catatan_ak03'])) {
    $id_ak03 = intval($_POST['id_ak03']);
    $catatan = $_POST['catatan_ak03'] ?? '';
    $stmt = mysqli_prepare($koneksi,
        "UPDATE tb_ak03 SET catatan_lainnya=? WHERE id_ak03=?");
    mysqli_stmt_bind_param($stmt, "si", $catatan, $id_ak03);
    mysqli_stmt_execute($stmt);
    header("Location: {$base}?page=../list/rekap_ak3.php");
    exit;
}

$filter = isset($_GET['filter']) ? $_GET['filter'] : 'semua';


$where = "WHERE 1=1";
if ($filter === 'belum')    $where .= " AND (a.catatan_lainnya IS NULL OR a.catatan_lainnya = '')";
if ($filter === 'sesuai')   $where .= " AND a.catatan_lainnya = 'Selesai'";
if ($filter === 'tidak')    $where .= " AND a.catatan_lainnya = 'Belum Selesai'";

$sql = "SELECT a.id_ak03, a.id_asesi, a.id_apl1, a.tgl_mulai, a.tgl_selesai, a.catatan_lainnya,
               s.judul_skema, s.nomor_skema,
               asi.nama_asesi
        FROM tb_ak03 a
        LEFT JOIN tb_apl1 apl ON apl.id_apl1 = a.id_apl1
        LEFT JOIN tb_skema s   ON s.id_skema  = apl.id_skema
        LEFT JOIN tb_asesi asi ON asi.id_asesi = a.id_asesi
        $where
        ORDER BY a.id_ak03 DESC";

$result = mysqli_query($koneksi, $sql);
$rows   = [];
while ($r = mysqli_fetch_assoc($result)) $rows[] = $r;
$total  = count($rows);

$total_all   = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) c FROM tb_ak03"))['c'] ?? 0;
$total_belum = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) c FROM tb_ak03 WHERE catatan_lainnya IS NULL OR catatan_lainnya=''"))['c'] ?? 0;
$total_sesuai = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) c FROM tb_ak03 WHERE catatan_lainnya='Selesai'"))['c'] ?? 0;
$total_tidak = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT COUNT(*) c FROM tb_ak03 WHERE catatan_lainnya='Belum Selesai'"))['c'] ?? 0;

$base = '../BERANDA/UTAMA.php';
?>
<style>
    .rekap-wrap { padding: 10px 4px; font-family: Arial, sans-serif; }
    .rekap-title { font-size: 20px; font-weight: bold; color: #1a237e; margin-bottom: 18px; }
    .rekap-cards { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 20px; }
    .rekap-card {
        flex: 1; min-width: 130px;
        border-radius: 8px; padding: 14px 16px;
        text-align: center; cursor: pointer;
        text-decoration: none; display: block;
        border: 2px solid transparent;
        transition: border 0.15s;
    }
    .rekap-card:hover { border-color: #4A7AFF; }
    .rekap-card.active { border-color: #4A7AFF !important; }
    .rekap-card .num { font-size: 26px; font-weight: bold; }
    .rekap-card .lbl { font-size: 12px; margin-top: 2px; }
    .card-semua { background: #e8eaf6; color: #1a237e; }
    .card-belum { background: #fff8e1; color: #e65100; }
    .card-sesuai { background: #e8f5e9; color: #2e7d32; }
    .card-tidak { background: #fce4ec; color: #c62828; }
    .rekap-table { width: 100%; border-collapse: collapse; font-size: 13px; }
    .rekap-table th {
        background: #cadbfc; padding: 9px 10px;
        border: 1px solid #b0bec5; text-align: center;
    }
    .rekap-table td {
        padding: 8px 10px; border: 1px solid #ddd; vertical-align: middle;
    }
    .rekap-table tr:hover td { background: #f5f7ff; }
    .badge {
        display: inline-block; padding: 2px 10px;
        border-radius: 20px; font-size: 11px; font-weight: bold;
    }
    .badge-belum { background: #fff8e1; color: #e65100; border: 1px solid #ffe082; }
    .badge-sesuai { background: #e8f5e9; color: #2e7d32; border: 1px solid #a5d6a7; }
    .badge-tidak { background: #fce4ec; color: #c62828; border: 1px solid #f48fb1; }
    .radio-group { margin-bottom: 6px; }
    .radio-group label { margin-right: 12px; font-size: 12px; cursor: pointer; }
    .komentar-textarea {
        width: 180px; padding: 5px; font-size: 11px;
        border-radius: 4px; border: 1px solid #ccc;
        resize: vertical; margin: 5px 0;
    }
    .btn-update {
        background: #f35555; color: white; border: none;
        padding: 5px 12px; border-radius: 4px; font-size: 11px;
        cursor: pointer;
        text-decoration: none;
        display: inline-block;
    }
    .btn-lihat {
        background: #4A7AFF; color: white; border: none;
        padding: 5px 12px; border-radius: 4px; font-size: 11px;
        cursor: pointer;
        text-decoration: none;
        display: inline-block;
    }
    .btn-update:hover { background: #8f4646; }
    .btn-lihat:hover { background: #325fd6; }
    .aksi-form { display: contents; }
    @media (max-width: 700px) {
        .komentar-textarea { width: 130px; }
        .rekap-table { font-size: 11px; }
    }
</style>

<div class="rekap-wrap">
    <div class="rekap-title">Rekap AK-03 — Umpan Balik dan Catatan Asesmen</div>

    <div class="rekap-cards">
        <a href="<?= $base ?>?page=../list/rekap_ak3.php&filter=semua"
           class="rekap-card card-semua <?= $filter==='semua'?'active':'' ?>">
            <div class="num"><?= $total_all ?></div>
            <div class="lbl">Total AK-03</div>
        </a>
        <a href="<?= $base ?>?page=../list/rekap_ak3.php&filter=belum"
           class="rekap-card card-belum <?= $filter==='belum'?'active':'' ?>">
            <div class="num"><?= $total_belum ?></div>
            <div class="lbl">Belum Diproses</div>
        </a>
        <a href="<?= $base ?>?page=../list/rekap_ak3.php&filter=sesuai"
           class="rekap-card card-sesuai <?= $filter==='selesai'?'active':'' ?>">
            <div class="num"><?= $total_sesuai ?></div>
            <div class="lbl">Selesai</div>
        </a>
        <a href="<?= $base ?>?page=../list/rekap_ak3.php&filter=tidak"
           class="rekap-card card-tidak <?= $filter==='belumdiselesaikan'?'active':'' ?>">
            <div class="num"><?= $total_tidak ?></div>
            <div class="lbl">Belum Selesai</div>
        </a>
    </div>

    <?php if (empty($rows)): ?>
        <div class="empty-msg">Tidak ada data untuk filter ini.</div>
    <?php else: ?>
    <div style="overflow-x:auto;">
        <table class="rekap-table">
            <thead>
                <tr>
                    <th>No.</th>
                    <th>Nama Asesi</th>
                    <th>Skema</th>
                    <th>Tgl Mulai</th>
                    <th>Tgl Selesai</th>
                    <th>Status</th>
                    <!-- <th>Komentar Admin</th> -->
                    <th>Aksi</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($rows as $i => $r): ?>
                <?php 
                    $is_belum = (is_null($r['catatan_lainnya']) || $r['catatan_lainnya'] === '');
                ?>
                <tr>
                    <td style="text-align:center;"><?= $i + 1 ?></td>
                    <td><?= htmlspecialchars($r['nama_asesi'] ?? '') ?></td>
                    <td>
                        <?= htmlspecialchars($r['judul_skema'] ?? '—') ?>
                        <div style="font-size:11px; color:#888;">No. <?= htmlspecialchars($r['nomor_skema'] ?? '—') ?></div>
                    </td>
                    <td style="text-align:center;"><?= htmlspecialchars($r['tgl_mulai'] ?? '—') ?></td>
                    <td style="text-align:center;"><?= htmlspecialchars($r['tgl_selesai'] ?? '—') ?></td>
                    <td style="text-align:center;">
                        <?php if ($is_belum): ?>
                            <span class="badge badge-belum">Belum Diproses</span>
                        <?php elseif ($r['catatan_lainnya'] === 'selesai'): ?>
                            <span class="badge badge-sesuai">selesai</span>
                        <?php else: ?>
                            <span class="badge badge-tidak">belum selesai</span>
                        <?php endif; ?>
                    </td>
                    <!-- <td style="max-width:200px;"> -->
                        <!-- <php if ($is_belum): ?>
                            <form method="post" action="<= $base ?>?page=../list/rekap_ak3.php&filter=<= urlencode($filter) ?>">
                                <input type="hidden" name="id_ak03" value="<= $r['id_ak03'] ?>">
                                <input type="hidden" name="update_status_ak03" value="1">
                                <textarea name="catatan_ak03" class="komentar-textarea"
                                          placeholder="Komentar admin (opsional)"><= htmlspecialchars($r['catatan_lainnya'] ?? '') ?></textarea>
                                     <div class="radio-group" style="margin-top:5px;">
                                    <label>
                                        <input type="radio" name="status_ak03" value="Sesuai"
                                            <= ($r['status'] === 'Sesuai') ? 'checked' : '' ?>>
                                        Sesuai
                                    </label>
                                    <label>
                                        <input type="radio" name="status_ak03" value="Tidak Sesuai"
                                            <= ($r['status'] === 'Tidak Sesuai') ? 'checked' : '' ?>>
                                        Tidak Sesuai
                                    </label>
                                </div> 
                                 <button type="submit" class="btn-update">Update</button>
                            </form>
                        <php else: ?>
                            <= nl2br(htmlspecialchars($r['catatan_lainya'] ?? '')) ?>
                        <php endif; ?> -->
                    <!-- </td> -->
                    <td style="text-align:left;">
                        <!-- <php if (!$is_belum): ?>
                            <a class="btn-lihat" href="<= $base ?>?page=../FR_APL/FR_AK03.php&view=1&id_asesi=<= $r['id_asesi'] ?>">
                                Lihat
                            </a>
                        <php endif; ?> -->
                        <a class="btn-lihat" href="<?= $base ?>?page=../FR_APL/FR_AK03.php&view=1&id_asesi=<?= $r['id_asesi'] ?>">
                            Lihat
                        </a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div style="font-size:12px; color:#888; margin-top:8px;">
        Menampilkan <?= $total ?> data
    </div>
    <?php endif; ?>
</div>
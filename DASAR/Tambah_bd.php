...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' && $id_skema_form <= 0) {
    $_SESSION['pesan'] = 'Tambah bukti dasar harus melalui daftar skema (tombol Tambah dari halaman Bukti Dasar).';
    $_SESSION['tipe'] = 'error';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah'])) {
    ...
    if ($bukti_dasar === '' || trim($_POST['bukti_dasar'] ?? '') === '') {
        $errors[] = "Bukti dasar harus diisi";
    }

    if (empty($errors)) {
        $check_sql = "SELECT id_bd FROM tb_bukti_dasar WHERE id_skema = ? AND bukti_dasar = ?";
        $check_stmt = mysqli_prepare($koneksi, $check_sql);
        mysqli_stmt_bind_param($check_stmt, "is", $id_skema_post, $bukti_dasar);
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            $insert_sql = "INSERT INTO tb_bukti_dasar (id_skema, bukti_dasar) VALUES (?, ?)";
            $insert_stmt = mysqli_prepare($koneksi, $insert_sql);

            if ($insert_stmt) {
                mysqli_stmt_bind_param($insert_stmt, "is", $id_skema_post, $bukti_dasar);
                ...
                try {
                    if (mysqli_stmt_execute($insert_stmt)) {
                        ...
                        header("Location: ../BERANDA/UTAMA.php?page=../DASAR/bukti_dasar.php&id_skema=" . $id_skema_post);
                        exit();
        ...
        <div class="form-container">
            <form method="post" action="" id="tambahUserForm">
                ...
                <div class="btn-container">
                    <a href="../BERANDA/UTAMA.php?page=../DASAR/bukti_dasar.php&id_skema=<?= (int) $id_skema_form ?>" class="btn btn-secondary">

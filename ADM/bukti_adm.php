...
if ($id_skema_param > 0) {
    $stmt = mysqli_prepare($koneksi, "SELECT * FROM tb_bukti_adm WHERE id_skema = ? ORDER BY id_ba ASC");
    mysqli_stmt_bind_param($stmt, "i", $id_skema_param);
    mysqli_stmt_execute($stmt);
    $hasil = mysqli_stmt_get_result($stmt);
    mysqli_stmt_close($stmt);
} else {
    $hasil = mysqli_query($koneksi, "SELECT * FROM tb_bukti_adm ORDER BY id_skema ASC, id_ba ASC");
}
            ...
            if (count($rows) > 0) {
                foreach ($rows as $row) {
                    ...
                    if ($can_manage) {
                        ...
                        $aksi = "<td data-label='Aksi' class='aksi'>—</td>";
                    }
                    echo $aksi;
                    echo "</tr>";
                }
